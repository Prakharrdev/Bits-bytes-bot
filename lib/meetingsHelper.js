...
async function parseScope(scopeStr, guild) {
	...
	const contributorRole = guild.roles?.cache?.get('1506019068132462804') ||
...
async function createMeetingVoiceChannel(guild, meeting) {
	try {
		...
		if (meeting.temp_channel_id) {
			...
			if (existingChannel) {
				// Auto-start/resume recording if enabled and not already recording
				if (process.env.RECORDING_ENABLED === 'true') {
					try {
						const { isRecording, startRecording } = require('./voiceRecorder');
						if (!isRecording(meeting.id)) {
							await startRecording(existingChannel, meeting.id, guild.client);
							console.log(`[MEETING] Resumed recording for existing channel "${meeting.title}" (${meeting.id})`);
						}
					} catch (err) {
						console.error(`[MEETING] Failed to resume recording for existing channel "${meeting.title}":`, err.message);
					}
				}
