...
async function startRecording(voiceChannel, meetingId, client) {
	...
	// ── Step 1: Play consent TTS if at least 2 people are in the voice channel ──
	...
	// ── Step 2: Send consent text in VC chat with Hindi button (individually for each user) ──
	...
	// ── Step 3: Subscribe to all present users' audio ──
...
function handleUserJoin(meetingId, member) {
	...
	// Play English consent audio notice if it hasn't been played yet and there are at least 2 people
	if (!session.hasPlayedConsentAudio) {
