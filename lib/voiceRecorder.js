...
async function startRecording(voiceChannel, meetingId, mainClient) {
	...
	if (listenerClient) {
		try {
			...
			recordingVoiceChannel = await listenerGuild.channels.fetch(voiceChannel.id);
		} catch (err) {
	...
	const presentMembers = mainVoiceChannel ? [...mainVoiceChannel.members.values()].filter(m => !m.user.bot) : [];
	...
	receiver.speaking.on('start', async (userId) => {
		...
		if (!sess.users.has(userId)) {
			try {
				const mainGuildObj = sess.mainClient.guilds.cache.get(voiceChannel.guild.id);
...
async function handleUserJoin(meetingId, member) {
	...
	const channel = member.guild.channels.cache.get(session.channelId) || await member.guild.channels.fetch(session.channelId).catch(() => null);
