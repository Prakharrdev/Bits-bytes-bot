		...
		for (const booking of bookings) {
			...
			if (!existingMeeting) {
				...
				if (scheduledTimeChanged) {
					...
					// Update database
					await meetingsDb.updateMeetingStatus(existingMeeting.id, 'scheduled'); // Reset status in case it was active/completed
					...
					// Delete sent reminders so reminders are sent again for the new time
