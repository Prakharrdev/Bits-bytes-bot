...
async function isAuthorizedForForkId(user, forkId, guild) {
	...
	try {
		...
		} else if (notion && typeof notion.retrievePage === 'function') {
			fork = await notion.retrievePage(forkId);
		} else if (notion && notion.client && notion.client.pages && typeof notion.client.pages.retrieve === 'function') {
			fork = await notion.client.pages.retrieve({ page_id: forkId });
		} else {
